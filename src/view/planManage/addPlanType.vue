<template>
  <div class="addPlanType">
    <h2 v-if="this.$route.params.type == 'add'">添加计划类型</h2>
    <h2 v-if="this.$route.params.type == 'edit'">编辑计划类型</h2>
    <Tabs>
      <TabPane label="基本信息">
      ...
      <TabPane label="关键词">
        <div class="keyWordStyle">
          <div class='loopAdd' v-for='(item, index) in keyWordAdd'>
            <span>关键词{{index+1}}</span>
            <Select v-model='item.content' style="width: 100px;margin: 0 20px" clearable>
              <Option v-for='(item, index) in keyWordSelectNum' :value="item.paramCode">{{item.paramName}}</Option>
            </Select>
      ...
      <TabPane label="相同类型设置">
        <div class="keyWordStyle">
          ...
          <div v-for='(item, index) in AddFollowSet' style='marginTop: 20px;display: flex'>
            <span style="marginTop: 7px">相同随访类型{{index+1}}</span>
            <Select v-model="item.select" style="width: 150px;margin: 0 20px" clearable>
              <Option v-for='(item,index) in followTypeNum' :value="item.planTypeId">{{item.planTypeName}}</Option>
            </Select>
            <Select v-model="item.timer" style="width: 200px;margin: 0 20px" clearable>
              <Option 
                v-for='(item, index) in yearSelectNum' 
                :value="`${item.paramId}&${item.paramCode}&${item.paramName}`">{{item.paramName}}</Option>
            </Select>
    ...
    async originTree(params){
      ...
      if(this.$route.params.type=='edit'){
         this.userList({
          ...
          planTypeId: this.$route.params.rowData.planTypeId 
        })
      } 
     // 如果有值或者下拉不为空
    ...
    async handleFollowType(){
      ...
      if(this.$route.params.type=='edit') {
        for(var i=0;i<this.followTypeNum.length;i++){
          if(this.followTypeNum[i].planTypeName == this.$route.params.rowData.planTypeName){
            this.followTypeNum.splice(i,1);
          } 
        }
      }
       // 出院小结下拉框
