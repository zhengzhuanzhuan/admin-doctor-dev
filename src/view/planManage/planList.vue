  ...
  <div class="planList">
    ...
    <Tabs @on-click='handleTabs'>
      <TabPane 
        ...
        <Form :model="formValue" :label-width="90" class='tableSearch'>
          ...
          <FormItem label="计划状态">
            <Select v-model="formValue.status" style="width: 180px;" clearable>
              <Option v-for='item in planStatusNum' :value="item.paramCode">{{item.paramName}}</Option>
            </Select>
          </FormItem>
        ...
        <div class="tableChoose">
          <Button @click="handleSelectPage(index)">全选当页</Button>
          <Button style="margin: 0 20px"  @click='handleSelectAll(index)'>全选所有页</Button>
    ...
    <Drawer width='600' title="计划详情" :closable="false" v-model="planDetail">
      <div>
        ...
        <div class='testList' v-for='(item,index) in planDetailNum.lists'>
          <Row>
            <Col span="6">
              <h1>第{{index+1}}次</h1>
            </Col>
            <Col span="13">
              ...
              <p>实际执行时间：{{item.recordBeginDate? parseTime(item.recordBeginDate) : ''}} <span style='color: #FF9900'>{{planStatusList[planDetailNum.status]}}</span></p>
              <h3>采集指标</h3>
              <Button type="primary" class="btn" v-for='text in item.targets'>{{text &&　text.targetName}}</Button>
            </Col>
          </Row>
        </div>
      </div>
      ...
      <Form ref="formValidate" :model="formValidate" :rules="validate.planlist" :label-width="80">
        <FormItem label="终止原因" prop="cancleReason">
          <Select v-model="formValidate.cancleReason" clearable>
            <Option v-for='(item, index) in stopReasonNum' :value="item.paramCode">{{item.paramName}}</Option>
          </Select>
        </FormItem>
...
export default {
  ...
  methods: {
    ...
    handleSelectPage(index){
      ...
      const planIds = this.tableData.map((item, index)=> (
        item.planId
      ))
    ...
    async handleTabLabel(){
      const res = await this.$API.planManage.planTypeList({pager: 1, limit: 999})
    ...
    async handlePlanDetail(data){
      ...
      console.log( this.planDetailNum )
    },
    // 页码改变
    handlePageNum(pager){
    ...
    selectTree(data){
      ...
      this.handlePlanList({organizationId: data[0].organizationId})
    }
  },
  mounted(){
    this.planStatus()
