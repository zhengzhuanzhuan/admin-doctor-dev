      ...
      <Form :model="formData" :rules="validate.menuEdit" ref="form" class="drawer-form" :label-width="80">
        ...
        <FormItem label="图标">
          <Select v-model="formData.menuLogo" clearable filterable :disabled="formData.menuType !== '0' || drawerType==='detail'">
            <Option v-for="(item, index) in menuIcons" :value="item" :key="index"><Icon :type="item" class="margin-right-10" />{{item}}</Option>
          </Select>
        </FormItem>
        <FormItem label="权限值">
...
export default {
     ...
     */
        async getSystemList () {
            ...
            if (res.data && res.data[0] && res.data[0].systemId) this.getMenuData(res.data[0].systemId)
        },
        /**
     * 某个系统的菜单数据
     * @param systemId
