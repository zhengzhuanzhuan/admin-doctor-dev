		...
		<Tabs value="1" @on-click="handleTabClick">
			<TabPane label="基本信息" name="1">
				<div class="tabOneContent">
					<Form class="base-form" ref="baseForm" :model="baseInfo" :rules="validate.userEdit" :label-width="120">
						...
						<FormItem label="专属号码" prop="mobile">
							<Input v-model.trim="baseInfo.mobile"></Input>
						</FormItem>
						<FormItem label="头像">
							<Upload
								action="/temp/posts/"
						...
						<FormItem label="职称">
							<Input v-model.trim="baseInfo.title"></Input>
						</FormItem>
						<FormItem label="简介">
							<Input type="textarea" :autosize="true" v-model.trim="baseInfo.introduce"></Input>
						</FormItem>
						<FormItem label="主要科室" prop="organizationId">
							<Cascader :data="orgOptions" change-on-select @on-change="handleOrgSelect" :value="editOrgArray" filterable></Cascader>
						</FormItem>
						...
						<FormItem label="是否禁用">
							<RadioGroup v-model="baseInfo.isEnable">
			...
			<TabPane label="角色" name="2">
				<div class="base-form">
					<CheckboxGroup v-model="chosenRoleArray">
						<Checkbox v-for="item in roleArray" :label="item.roleId" :key="item.roleId">
			...
			<TabPane label="关联用户数据" name="4">
				<Row class="user_main_search common_search" :gutter="20">
					...
					<Col span="6">
						<span>状态</span>
						<Select v-model="searchParamsTabFour.isRelation">
				...
				<Row class="user_main_page">
					<Page :total="tableParamsFour.total" :page-size="searchParamsTabFour.limit" show-total :current="searchParamsTabFour.pager" @on-change="handlePage('Four', $event)"></Page>
				</Row>
			</TabPane>
			<TabPane label="方案" name="5">
				...
				<Row class="user_main_search common_search" :gutter="20">
					...
					<Col span="6">
						<span>疾病类型</span>
						<!--<Input v-model="searchParamsTabFive.diseaseName"></Input>-->
				...
				<div class="user_main_list">
					<Table :columns="configFive" :data="tableParamsFive.data" :loading="tableParamsFive.isLoading"></Table>
				</div>
				<Row class="user_main_page">
					<Page :total="tableParamsFive.total" :page-size="searchParamsTabFive.limit" show-total :current="searchParamsTabFive.pager" @on-change="handlePage('Five', $event)"></Page>
				</Row>
				<div class="save-wrapper">
			...
			<TabPane label="疾病" name="6">
				...
				<Row class="user_main_search common_search" :gutter="20">
					<Col span="6">
						...
						<Select v-model="searchParamsTabSix.typeCode" clearable>
							<Option v-for="item in searchTypeOptions" :value="item.paramCode" :key="item.paramId">{{ item.paramName }}</Option>
						</Select>
					</Col>
					<Col span="6">
						<span>名称</span>
				...
				<div class="user_main_list">
					<Table :columns="configSix" :data="tableParamsSix.data" :loading="tableParamsSix.isLoading"></Table>
				</div>
				<Row class="user_main_page">
					<Page :total="tableParamsSix.total" :page-size="searchParamsTabSix.limit" show-total :current="searchParamsTabSix.pager" @on-change="handlePage('Six', $event)"></Page>
				</Row>
				<div class="save-wrapper">
...
export default {
                        ...
                        return h('div', [
                            h(
                                'Button',
                                {
                                    props: {
                                        size: 'small',
                                        type: params.row.isRelation == 1 ? 'primary' : 'default'
                                    },
                                    on: {
                                        click: () => {
                                ...
                                params.row.isRelation == 1 ? '已关联' : '关联数据'
                            )
                        ])
                    }
                }
    ...
    methods: {
		 ...
		 */
        async getBaseInfo () {
            const res = await this.$API.userManage.allInfo({ id: this.baseInfo.userId })
            ...
            this.editOrgArray = getCascaderSelectedValue(this.orgOptions, this.baseInfo.organizationId)
            // tab2 赋值
        ...
        async getOrgOptions (parentId = 0) {
            const res = await this.$API.organizeManage.list({ pager: 1, limit: 9999, parentId: '' })
            ...
            for (let i = 0; i < resultCustomParamsArray.length; i++) {
                ...
                if (item) {
                    if (item.value === '' && item.type !== 'image' && item.required === 1) {
                        ...
                        formData.append(`upmsFieldDynamicList[${i}].fieldDynamicId`, item.fieldDynamicId || '')
                        formData.append(`upmsFieldDynamicList[${i}].extendObjectId`, item.extendObjectId || '')
                        formData.append(`upmsFieldDynamicList[${i}].fieldId`, item.fieldId)
                        formData.append(`upmsFieldDynamicList[${i}].fieldName`, item.fieldName)
                        formData.append(`upmsFieldDynamicList[${i}].fieldCode`, item.code)
                        formData.append(`upmsFieldDynamicList[${i}].fieldSort`, item.sort)
                        formData.append(`upmsFieldDynamicList[${i}].fieldType`, item.type)
                        formData.append(`upmsFieldDynamicList[${i}].fieldValue`, item.value)
                    }
                }
            }
                ...
                if (valid) {
                    ...
                    for (let key in this.baseInfo) {
                    	...
                    	if (key === 'password' && this.baseInfo.userId) {
                            formData.append(`upmsUser.password`, this.baseInfo.updatePassword || '')
                        } else {
                            formData.append(`upmsUser.${key}`, this.baseInfo[key])
                        }
                    }
                    formData.append('logo', this.baseInfoLogo)
                    // 自定义添加的图片
                    ...
                    if (!this.baseInfo.userId) {
                        ...
                        this.baseInfo.userId = postRes.data.userId // 为之后的几个tab存下userId
                        this.getBaseInfo()
                    } else {
        ...
        // tab2 methods--start
        async getRoleList () {
        	const res = await this.$API.roleManage.list({ limit: 9999, pager: 1 })
        ...
        handleOrgSelectTabFour (value, selectData) {
            ...
            this.searchParamsTabFour.organizationId = userfulData ? userfulData.value : ''
        },
        async handleToggleChoose (rowData) {
        	...
        	if (rowData.isRelation == 0) {
        		postData.relationUserId = rowData.userId
                await this.$API.userManage.updateUserData(postData)
                rowData.isRelation = 1
            } else {
                await this.$API.userManage.deleteUserData(postData)
                rowData.isRelation = 0
            }
        },
        // tab 4 关联用户数据-end
        // tab 5 方案-start
        handleOrgSelectTabFive (value, selectData) {
            const userfulData = selectData[selectData.length - 1]
            this.searchParamsTabFive.organizationId = userfulData ? userfulData.value : ''
        },
        // tab 5 方案-end
        // tab 6 疾病-start
        ...
        async handleSave (type) {
            ...
            } else if (type === 'orgList') { // 该项的allData[]数据无效
        ...
        async getTableData (name) {
            ...
            try {
                if (name === 'Four') {
                    ...
                    searchParam.diseaseType = disSelectRes.firstData ? disSelectRes.firstData.paramId : ''
                    searchParam.diseaseId = disSelectRes.secondData ? disSelectRes.secondData.paramId : ''
                    res = await this.$API.schemeManage.list(searchParam)
                    res.data = this.getChosenData(res.data, this.chosenPlanArr, 'schemeId')
                } else if (name === 'Six') {
                    res = await this.$API.diseaseManage.list(searchParam)
                    res.data = this.getChosenData(res.data, this.chosenDiseaseArr, 'diseaseId')
                }
            } catch (e) {
        		this[tableName].isLoading = false
            }
            this[tableName].isLoading = false
            this[tableName].data = res.data
            this[tableName].total = res.total
        },
        getTableDataFour () {
        ...
        handleTagDelete (index, arrName) {
            ...
            if (arrName === 'chosenPlanArr') {
                this.tableParamsFive.data = this.getChosenData(this.tableParamsFive.data, this.chosenPlanArr, 'schemeId')
            } else if (arrName === 'chosenDiseaseArr') {
                this.tableParamsSix.data = this.getChosenData(this.tableParamsSix.data, this.chosenDiseaseArr, 'diseaseId')
            }
        }
    },
    mounted () {
        ...
        // console.log(this.baseInfo.updatePassword)
        this.getTabOneData()
    }
}
</script>
