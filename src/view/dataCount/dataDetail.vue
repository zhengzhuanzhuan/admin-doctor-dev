<template>
  <div class="dataDetail">
    <Form :model="tableParams" :label-width="120">
       <FormItem  v-for="(item,index) in mouldCondition" :label="item.conditionFieldName" style="display: inline-block">
          ...
          <!-- <div style='display:flex;marginTop: 10px'>
            <Checkbox
              ...
              <CheckboxGroup v-model="checkAllGroup" @on-change="checkAllGroupChange">
                <Checkbox v-for='(item,index) in schemeTypeNum' :label="item.planTypeId">{{item.planTypeName}}</Checkbox>
              </CheckboxGroup>
          </div> -->
        </div>
         <!-- 人工外呼意见-->
        ...
        <div v-if="item.conditionField=='organizationId'" style="display: flex;width:75vw">
          ...
          <CheckboxGroup v-model="departmentGroup" @on-change="checkAllGroupChange($event,'departmentNum', 'indeDepartment','checkDepartment')">
            <Checkbox v-for='(item,index) in departmentNum' :label="item.organizationId">{{item.name}}</Checkbox>
          </CheckboxGroup>
        </div>
         <!-- 通话状态 -->
        <div v-if="item.conditionField=='callStatus'" style="display: flex;width:75vw">
          ...
          <CheckboxGroup v-model="callStatusGroup" @on-change="checkAllGroupChange($event,'callStatusNum', 'indeCallStatus','checkCallStatus')">
            <Checkbox v-for='(item,index) in callStatusNum' :label="item.paramCode">{{item.paramName}}</Checkbox>
          </CheckboxGroup>
        </div>
        <!-- 方案类型多选 -->
          ...
          <div style='display:flex;marginTop: 10px'>
            <Checkbox
              ...
              <CheckboxGroup v-model="schemeNameGroup" @on-change="checkAllGroupChange($event,'schemeCheckNum', 'indeSchemeName','checkSchemeName')">
                <Checkbox v-for='(item,index) in schemeCheckNum' :label="item.schemeId">{{item.schemeName}}</Checkbox>
              </CheckboxGroup>
          </div>
        </div>
         <!-- 方案名称单选 -->
