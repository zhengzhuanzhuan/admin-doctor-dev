		...
		<Row class="user_main_search common_search" :gutter="20">
			<Col span="5">
				...
				<Select v-model="searchParams.followupType" clearable>
					<Option v-for="item in followupTypeOptions" :value="item.planTypeId">{{item.planTypeName}}</Option>
				</Select>
			</Col>
			<Col span="5">
				<span>患者姓名</span>
			...
			<Col span="5">
				<span>外呼人</span>
				<Input v-model="searchParams.callPeople" placeholder="患者姓名"></Input>
			</Col>
			<Col span="5">
				<span>外呼状态</span>
				<Select v-model="searchParams.outCallStatus" clearable>
					<Option v-for="item in callStateNum" :value="item.paramCode">{{item.paramName}}</Option>
				</Select>
			</Col>
			<Col span="6">
				<span>外呼时间</span>
			...
			<Col span="5">
				...
				<Select v-model="searchParams.callStatus" clearable>
					<Option v-for="item in teleStateNum" :value="item.paramCode">{{item.paramName}}</Option>
				</Select>
			</Col>
			<Col span="2">
              ...
              <Form ref="stopForm" :model="dealBaseMes" :rules="ruleValidate" :label-width="120">
                <FormItem label="人工外呼通话状态" prop="callStatus">
                  <Select v-model="dealBaseMes.callStatus" placeholder="请选择通话状态" :disabled='dealBaseMes.callStatus==10' style="width: 200px">
                    <Option v-for='(item, index) in teleStateNum' :value="item.paramCode">{{item.paramName}}</Option>
                  </Select>
                </FormItem>
              </Form>
            </div>
