<template>
  <div class="outCall">
    <Form ref="formValidate" :model="formValidate" :label-width="80"  class="outCall-search">
      <FormItem label="随访类型">
        <Select v-model="formValidate.followupType" placeholder="请选择随访类型" style="width:200px" clearable>
          <Option v-for='(item, index) in followTypeNum' :value="item.planTypeCode">{{item.planTypeName}}</Option>
        </Select>
      </FormItem>
      <FormItem label="姓名">
      ...
      <FormItem label="外呼状态">
        <Select v-model="formValidate.outCallStatus" placeholder="请输入外呼状态" style="width:200px" clearable>
          <Option v-for='(item,index) in outCallStatusNum' :value="item.paramCode">{{item.paramName}}</Option>
        </Select>
      </FormItem>
      <FormItem label="外呼时间">
        <date-time-picker 
      ...
      <FormItem label="通话状态">
        <Select v-model="formValidate.callStatus" placeholder="请选择通话状态" style="width:200px" clearable>
          <Option v-for='(item, index) in callStatusNum' :value="item.paramCode">{{item.paramName}}</Option>
        </Select>
      </FormItem>
      <FormItem>
    ...
    <Modal title="随访详情" width="780" v-model='handleModal' :mask-closable='false'>
      <Collapse value="1">
        <Panel name="1">
          随访结果
          <div slot="content" class="baseMes">
            ...
            <Row class="rowSpace">
              <Col span="8">执行时间：{{dealBaseMes.date}}</Col>
										...
										<li>
											...
											<div class="result-wrap">
												...
												<RadioGroup v-else v-model="item.fieldValue" transfer>
													 <Radio v-for='text in item.targetOptions' :label='text.optionValues'></Radio>
												</RadioGroup>
											</div>
										</li>
										<li>
...
export default {
    ...
    return {
      ...
      columnsData: [
        ...
        },
        {
          ...
          render: (h, params) => {
            return h('div', [
              h('p', {}, params.row.outCallStatus==0? '': params.row.outCallPeople),
            ]);
          }
        },
        {
        ...
        },
        {
          ...
          render: (h, params) => {
            let text = ''
            if(params.row.outCallStatus == 1){
              text = '已外呼'
            } else if(params.row.outCallStatus==0){
              text = '等待外呼'
            }
            return h('div', [
              h('p', {
        ...
        },
        {
          ...
          render: (h, params) => {
            return h('div', [
              h('p', {
              }, this.callStateColum[params.row.callStatus]),
            ]);
          }
        },
        {
  ...
  methods: {
    ...
    selectTree(data){
      ...
      this.handleCallList({organizationId: data[0].organizationId})
    },
