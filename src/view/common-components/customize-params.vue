<template>
	<div class="customize-params">
		...
		<template v-for="(item, index) in customParamsArray">
				<FormItem :label="item.fieldName" :required="item.required === 1" :key="item.fieldId" v-if="item">
					<Input v-model.trim="item.value" v-if="item.type==='text'" :disabled="!showDelete"></Input>
					<InputNumber v-model="item.value" v-else-if="item.type==='numerical'" :disabled="!showDelete"></InputNumber>
					<Select v-model="item.value" v-else-if="item.type==='select'" :disabled="!showDelete">
						<template v-if="item.typeParameters">
							<Option v-for="(opt, optIndex) in item.typeParameters.split(',')" :value='opt' :key="optIndex">{{ opt }}</Option>
						</template>
					...
					>
						<Button icon="ios-cloud-upload-outline" @click="getParamUploadIndex(index)" :disabled="!showDelete">上传图片</Button>
					</Upload>
					<Button icon="ios-close" shape="circle" class="delete-btn" @click="handleDelete(index)" v-if="showDelete"></Button>
		...
		<!--基本信息-添加字段 按钮点击后的弹框-->
		<Drawer title="添加字段" :mask-closable="false" v-model="showDrawer" width="560" class-name="field-drawer">
...
export default {
    ...
    props: {
        baseParamsArray: { // 左侧选中的组织信息
            type: Array,
            default: function () {
            	return []
            }
        },
        showDelete: { // 查看时，设置为false，input置为disable,不显示 删除 按钮
        	type: Boolean,
            default: true
        }
    },
    watch: {
        baseParamsArray (val) {
            ...
            val.forEach((item, index) => {
            	if (item) {
                    ...
                    if (item.fieldType === 'select') {
                        this.$API.fieldManage.list({ fieldId: item.fieldId, limit: 9999 }).then(res => {
            ...
            // console.warn(this.customParamsImgs)
        }
    },
    methods: {
    	returnData () {
            if (this.isImageAllUpload()) {
                return {
        ...
        /**
		 * @description 判断 图片类型字段是否都已上传
		 */
        isImageAllUpload () {
        	// 删除字段操作后，部分customParamsImgs属性值为空
            // const fileLength = Object.getOwnPropertyNames(this.customParamsImgs).length - 1
            let fileLength = 0
            for (let key in this.customParamsImgs) {
        		if (this.customParamsImgs[key]) {
        			fileLength++
                }
            }
            if (this.customParamsArray.length > fileLength) {
                let fileNum = 0
                ...
                if (fileNum > fileLength) {
                    this.$Message.warning('自定义的图片类型字段，必须上传图片！请先上传对应图片')
                    return false
                }
            }
            return true
        ...
        baseInfoOtherParam (data) {
            ...
            for (let i = 0; i < paramsLength; i++) {
                if (this.customParamsArray[i] && data.fieldId === this.customParamsArray[i].fieldId) {
